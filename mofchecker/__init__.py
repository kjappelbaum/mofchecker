...
class MOFChecker:  # pylint:disable=too-many-instance-attributes, too-many-public-methods
    ...
    def _has_undercoordinated_nitrogen(self, tolerance: int = 15):
        ...
        for site_index in self.n_indices:
            ...
            if cn == 1:
                ...
                # linear sp one
                if (self.get_cn(neighbors[0].index) >
                        2) and not neighbors[0].site.specie.is_metal():
    ...
    def get_cn(self, site_index) -> int:
        ...
        if site_index not in self._cns:
            with warnings.catch_warnings():
                ...
                self._set_cnn()
                self._cns[site_index] = self._cnn.get_cn(
                    self.structure, site_index)
    ...
    def is_site_open(self, site_index: int) -> bool:
        ...
        if site_index not in self._open_indices:
            try:
                _, _names, lsop, is_open, weights = self._get_ops_for_site(
                    site_index)
    ...
    def get_mof_descriptors(self) -> dict:
        ...
        result_dict = OrderedDict(
            (("name", self.name), ("path", self._filename), ("density",
                                                             self.density),
             ("has_oms", self.has_oms), ("has_carbon", self.has_carbon),
             ("has_hydrogen", self.has_hydrogen), ("has_atomic_overlaps",
                                                   self.has_atomic_overlaps),
             ("has_overcoordinated_c",
              self.has_overvalent_c), ("has_overcoordinated_n",
                                       self.has_overvalent_n),
             ("has_overcoordinated_h",
              self.has_overvalent_h), ("has_undercoordinated_c",
                                       self.has_undercoordinated_c),
             ("has_undercoordinated_n",
              self.has_undercoordinated_n), ("has_metal", self.has_metal),
             ("has_lone_atom", self.has_lone_atom), ("has_lone_molecule",
                                                     self.has_lone_molecule),
